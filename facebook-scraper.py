...
def getPostIds(driver, filePath='posts.csv'):
    ...
    # Cập nhật xpath để tìm các liên kết bài đăng
    postLinks = driver.find_elements_by_xpath('//a[contains(@href, "/posts/")]')
    
    if len(postLinks):
        for link in postLinks:
            href = link.get_attribute('href')
            # Trích xuất ID bài đăng từ URL
            postId = href.split('/posts/')[1].split('?')[0]
            if postId not in allPosts:
                print(postId)
                writeFileTxt(filePath, postId)
    else:
        print("Không tìm thấy liên kết bài đăng nào.")

# Cập nhật hàm getnumOfPostFanpage

def getnumOfPostFanpage(driver, pageId, amount, filePath='posts.csv'):
    driver.get(f"https://touch.facebook.com/{pageId}")
    while len(readData(filePath)) < amount:
        getPostIds(driver, filePath)
        # Thêm logic để cuộn trang
        driver.execute_script("window.scrollTo(0, document.body.scrollHeight);")
        sleep(2)  # Đợi trang tải thêm nội dung


try:
    cookie = 'c_user=100086901612327; xs=35%3AuJ9es-gr8tBUsQ%3A2%3A1720365753%3A-1%3A-1'
    driver = initDriver()
    isLive = checkLiveCookie(driver, cookie)
    if isLive:
        getnumOfPostFanpage(driver, 'thinhseu.official', 1, 'posts.csv')
        for postId in readData('posts.csv'):
            getAmountOfComments(driver, postId, 10)
    else:
        logging.error("Cookie không hợp lệ hoặc đã hết hạn")
